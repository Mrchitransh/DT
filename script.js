...
const same_box = document.querySelectorAll(".same_box");
...
const list_item = document.querySelectorAll(".list_item li");
explore.innerHTML = `<h3>${Data.tasks[0].task_title}</h3>
<p>${Data.tasks[0].task_description}</p>`;
for (let i = 0; i < same_box.length; i++) {
  same_box[i].innerHTML = ` <p class="box_heading">
                ${Data.tasks[0].assets[i].asset_title} <span>i</span>
              </p>
              
              <p>
                <span>Description:</span> ${Data.tasks[0].assets[i].asset_description}
              </p>`;
}

video.innerHTML = ` <iframe src=${Data.tasks[0].assets[0].asset_content} width="450"  height="300" ></iframe>`;

right_arrow.addEventListener("click", () => {
  side_ul.classList.toggle("hide");
  one.classList.toggle("open_one");
  number.classList.toggle("hide");
  left_arrow.classList.toggle("hide");
});
list1.innerHTML = ` <li>${Data.tasks[0].task_title}</li>`;

for (let i = 0; i < list_item.length; i++) {
  list_item[i].innerHTML = `<li>${Data.tasks[0].assets[i].asset_title}</li>`;
}
